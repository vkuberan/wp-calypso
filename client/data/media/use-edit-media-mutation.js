...
export function useEditMediaMutation( queryOptions ) {
	return useMutation(
		( { siteId, mediaId, payload } ) => wp.site( siteId ).media( mediaId ).edit( payload ),
		queryOptions
	);
}
...
export const withEditMedia = createHigherOrderComponent(
	( Wrapped ) => ( props ) => {
		...
		const { mutate } = useEditMediaMutation( {
			onMutate( { mediaId } ) {
		...
		const editMedia = useCallback(
			( siteId, item ) => {
				...
				if ( originalMediaItem ) {
					mutate( { siteId, mediaId: item.ID, payload: item, originalMediaItem } );
				}
			},
			[ dispatch, mutate ]
		);
